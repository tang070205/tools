...
if dic == 'x' or dic == 'y':
    ...
    def plot_running_avg(data, subplot_index, color, ylabel, y_start, y_end, yticks, title_tag):
        ...
        plt.gca().set_yticks(np.arange(y_start, y_end+1, yticks))
        plt.xlabel('time (ns)')
        plt.ylabel(ylabel)
    ...
    plot_running_avg(ko_data, 2, 'blue', r'$\kappa_{out}$ W/m/K', 0, 4000, 1000, 'b')
    ...
    plt.ylim([0, 4000])
    plt.gca().set_xticks(np.arange(0, xlimit+1, 1))
    plt.gca().set_yticks(np.arange(0, 4001, 1000))
    plt.xlabel('time (ns)')
