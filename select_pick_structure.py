    ...
    for i in range(len(append_des)):
        if len(append_des[i]) == len(strucs[i]):
            if all_points_plot == 'combine':
                ...
                sc = scatter(all_proj[:, 0], all_proj[:, 1], c=energy_train, cmap='Blues')
            else:
            ...
            title(f'Descriptors for each atom in {xyz_file[i]} with {sys.argv[2]}')
            set_tick_params()
            xlabel('PC1')
            ylabel('PC2')
            tight_layout()
            savefig(f"each-elements-{xyz_file[i]}-{sys.argv[2]}.png", dpi=150, bbox_inches='tight')
    if len(append_des[0]) == len(strucs[0]):
        cbar = colorbar(sc, cax=gca().inset_axes([0.7, 0.95, 0.27, 0.02]), orientation='horizontal')
        cbar.ax.xaxis.set_major_formatter(FormatStrFormatter('%.1f'))
        cbar.ax.tick_params(axis='x', labelsize=10)
        cbar.set_label('E/atom (eV)', fontsize=10)
        legend(ncol=5, frameon=False, fontsize=8, loc='upper left')
